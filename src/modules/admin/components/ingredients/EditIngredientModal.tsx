...
import { editIngredientSchema, type EditIngredientSchema } from '../../schemas/edit-ingredient.schema';
...
import type { EditIngredientModalProps } from '../../types/ingredients-components-type';

export default function EditIngredientModal({ open, onClose, ingredientId, onUpdated }: EditIngredientModalProps) {
    const { ingredient, loading: loadingIngredient, error: errorIngredient, refetch } = useGetIngredient();
    const { updateIngredient, loading, error } = useUpdateIngredient();
    const { register, handleSubmit, formState: { errors }, reset, setValue } = useForm<EditIngredientSchema>({
        resolver: zodResolver(editIngredientSchema),
    });

    useEffect(() => {
        if (open && ingredientId) {
        refetch(ingredientId);
        }
    }, [open, ingredientId, refetch]);
    ...
    const onSubmit = async (data: EditIngredientSchema) => {
        if (!ingredientId) return;
        const updated = await updateIngredient(ingredientId, data);
        if (updated) {
        reset();
        onUpdated?.();
        onClose();
    ...
    return (
        ...
        >
            ...
            <h2 className="font-extrabold text-xl text-foreground">Editar ingrediente</h2>
            </div>
            {loadingIngredient ? (
            <div className="text-center py-8">Cargando...</div>
            ) : errorIngredient ? (
            <div className="text-red-500 text-center py-8">{errorIngredient}</div>
            ) : (
